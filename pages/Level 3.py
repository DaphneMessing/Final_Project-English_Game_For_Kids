...
# Function to get the word from the text file based on TV show and index
def get_word(tv_show, word_index):
    file_path = os.path.join(os.path.dirname(__file__), '..', 'words_level3.txt')
    with open(file_path, "r") as f:
        lines = f.readlines()
        words = [line.strip() for line in lines if line.startswith(tv_show)]
        if word_index < len(words):
            return words[word_index].split(":")[1].strip()
    return None
...
def main():
    ...
    word = get_word(show, st.session_state.word_index)
    ...
    if word:
        ...
        st.markdown(f'<div class="text-container"><p>Listen to the word:</p>', unsafe_allow_html=True)

        # Prepare the audio file path based on the selected TV show and word index
        show_no_spaces = show.replace(" ", "")
        audio_file = os.path.join(os.path.dirname(__file__), '..', 'audio_level3', f"{show_no_spaces}_{st.session_state.word_index}.mp3")
        ...
        if submit_button:
            ...
            if cleaned_user_input == cleaned_word:
                message_placeholder.markdown('<div class="success-fail" style="color: green;">Correct! Moving to the next word.</div>', unsafe_allow_html=True)
                time.sleep(2)  # Pause for 2 seconds before moving to the next word
