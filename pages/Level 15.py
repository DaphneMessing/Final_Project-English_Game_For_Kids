...
#     file_path = os.path.join(os.path.dirname(__file__), '..', 'sentences_level15.txt')
...
#     points = score * 100
#     st.session_state['level15_score'] = score
#     st.session_state['level15_points'] = points
...
#     st.markdown(f'<div class="text-container"><div class="level-heading">Level 15</div>', unsafe_allow_html=True)
...
#         audio_file = os.path.join(os.path.dirname(__file__), '..', 'audio_level15', f"{show_no_spaces}_{st.session_state.sentence_index}.mp3")
...
#         # Check if the "Submit" button was pressed or auto-submitted
#         if submit_button:
...
#         points = st.session_state['level15_points']
...
#         for i in range(1, 4):
#             star_class = 'star' if i <= st.session_state['level15_score'] else ''
